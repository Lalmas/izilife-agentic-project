"""
Agent Community Manager — izilife v4 simplifié
...
TYPE_CONTENT = {
    "POST_AGENDA_SEMAINE":"story", "POST_AGENDA_WEEKEND":"story",
    "POST_TOP_LIEUX":"carrousel", "POST_TOP_ACTEURS":"carrousel",
    "POST_HISTOIRE":"post", "POST_HUMOUR":"post",
    "POST_ESCAPADE_VILLE":"carrousel", "POST_ESCAPADE_NATURE":"carrousel",
    "POST_PEPITE":"post", "EVENT_SERIE":"post", "EVENT_ANIMATEUR":"post",
    "PARTAGE_ACTEUR":"post", "NOUVEAUTE_IZILIFE":"post", "REEL":"reel", "VIDEO":"video",
}
TYPE_TEMPLATE = {
...
def zone_config(zone: str, env_name: str = DEFAULT_SOCIAL_ENV) -> dict:
    ...
    return {"zone": z, "drive": drive, "excel": drive / f"planning_{z}.xlsx", "outputs": drive / "outputs", "inputs": drive / "inputs", "context_zone": CONTEXT_ROOT / "zones" / f"{z}.md"}
...
def run_zone(zone: str, env_name: str = DEFAULT_SOCIAL_ENV, dry_run=False):
    ...
    print(f"🤖 CM izilife V4 — {zone} — {len(rows)} ligne(s)" + (" [DRY RUN]" if dry_run else ""))
