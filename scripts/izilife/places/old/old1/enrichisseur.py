...
Fichiers Drive (zone) :
    ameliorateur.xlsx   → Onglet Prioritaires : url_izilife | notes | statut
...
PRIO_COLS = ["url_izilife", "notes", "statut"]
...
def create_ameliorateur(zone: str):
    ...
    _make_header(ws, PRIO_COLS, [60, 40, 14])
    exemples = [
        ["https://izilife.co/place/le-baron-club", "À enrichir en priorité", "pending"],
        ["https://izilife.co/shop/big-luck",        "",                       "pending"],
        ["", "", "pending"],
    ]
    for r, row in enumerate(exemples, 2):
        for c, val in enumerate(row, 1):
            ws.cell(row=r, column=c, value=val)
        _style_row(ws, r, 3, alt=(r%2==0))
    _add_dv(ws, "C", '"pending,done,error,skip"')
    _add_cf(ws, "A2:C2000", "C", {"done":"C6EFCE","error":"FFC7CE","skip":"D9D9D9"})
    wb.save(f)
...
def read_prioritaires(zone: str, env: dict) -> list[dict]:
    ...
    for r_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), 2):
        ...
        if data.get("statut","").lower() in ("done","error","skip"): continue
        url = data.get("url_izilife","")
        if not url: continue

        # Résoudre string_id et type depuis l'URL
        entity_type = "PLACE"
        string_id   = url
        if "/place/" in url:
            entity_type = "PLACE"
        ...
        if not resp or not resp.get("success"):
            log(f"  ⚠️  Lieu introuvable : {string_id}")
            continue
        item = resp["item"]
        item["_row_idx"] = r_idx
        item["_file"]    = f
...
def purge_done(zone: str, script: str = "enrichisseur"):
    ...
    lws = lb.active
    lws.title = "Done"
    _make_header(lws, headers, [60,40,14])
...
def scrape_google_maps(page, name: str, city: str = "") -> str:
    ...
    try:
        ...
        accept_cookies(page)
        sleep_rnd(1, 2)
        # Cliquer premier résultat si liste
        try:
            first = page.locator('a[href*="/maps/place/"]').first
            if first.is_visible(timeout=2000): first.click(); sleep_rnd(2, 3)
        except: pass
        try: page.wait_for_selector("h1", timeout=8000)
        except: pass
        # Scroller pour charger "À propos"
        page.evaluate("window.scrollBy(0, 500)")
        sleep_rnd(1, 2)
        text = page.evaluate("() => document.body.innerText")
        return f"=== GOOGLE MAPS ===\n{text[:8000]}"
    except Exception as e:
        log(f"    ⚠️  Google Maps: {e}"); return ""
...
def enrich_one(lieu: dict, page, env: dict, dry_run: bool, stats: dict):
    ...
    if not gid:
        log("    → Pas de google_place_id — appel postAgentImproveFromGoogle")
...
def main():
    ...
    if not args.priority_only:
        ...
        if dry_run and not normal:
            normal = [
                {"id":1,"type":"SHOP","name":"Test Bar","google_place_id":"","website":"","completion_score":35,"city_id":city_id},
            ]
