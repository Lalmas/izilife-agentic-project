...
Usage :
    python createur_lieux.py --zone=lille --init
...
def get_log_file(zone: str, script: str = "createur_lieux") -> Path:
    ...
    d = get_drive_root() / "agentic_workspace" / "izilife" / "places" / f"{zone}-zone" / "logs" / week
    d.mkdir(parents=True, exist_ok=True)
    return d / f"{script}_{today.isoformat()}.xlsx"


def purge_done(zone: str):
    pfile = get_curate_file(zone)
    if not pfile.exists():
    ...
    for attempt in range(3):
        try:
            ...
            for col, h in enumerate(headers, 1):
                lws.cell(row=1, column=col, value=h.upper())
            for r, row in enumerate(done_rows, 2):
                for c, val in enumerate(row, 1):
                    lws.cell(row=r, column=c, value=val)
            lb.save(lf)
            log(f"✅ {len(done_rows)} lignes → {lf.name}")

            # Réécrire sans les done
            ...
            wb.save(pfile)
            log(f"✅ {len(keep_rows)} lignes restantes")
            return
        except PermissionError:
            import time; time.sleep(3)
    log("❌ Fichier verrouillé — purge impossible")
