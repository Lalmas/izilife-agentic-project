...
---------------------
Scrape les events Eventbrite pour une ville et envoie vers izilife.
Pattern identique à shotgun_scraper.py.

Usage :
    python eventbrite_scraper.py --env=local --city=lille --dry-run
    python eventbrite_scraper.py --env=local --city=lille --zone=lille --pages=5

Note :
    Eventbrite bloque agressivement le scraping.
    En cas de blocage : utiliser les cookies d'une session authentifiée
    (même approche que facebook_scraper.py avec fb_cookies.json).
...
# Cookies Eventbrite optionnels (même format que fb_cookies.json)
COOKIES_PATH = Path(__file__).parent / "profiles" / "eventbrite" / "eb_cookies.json"

ZONES = {
    "lille":    "https://www.eventbrite.fr/d/france--lille/events/",
    "paris":    "https://www.eventbrite.fr/d/france--paris/events/",
    "lyon":     "https://www.eventbrite.fr/d/france--lyon/events/",
    "bordeaux": "https://www.eventbrite.fr/d/france--bordeaux/events/",
}

DELAY_PAGES  = (4, 8)   # plus long car Eventbrite bloque plus
DELAY_EVENTS = (3, 6)
...
def load_cookies(context):
    if COOKIES_PATH.exists():
        ...
        log(f"  🍪 {len(cookies)} cookies Eventbrite chargés")

def extract_event_urls(html: str) -> list:
    """Extrait les URLs d'events Eventbrite."""
    patterns = re.findall(
        r'href=["\']((https://www\.eventbrite\.fr)?/e/[^"\'?\s]+)["\']',
        html
    )
    seen = {}
    for path, base in patterns:
        url = path if path.startswith('http') else f"https://www.eventbrite.fr{path}"
...
def main():
    ...
    log(f"=== eventbrite_scraper.py — zone={args.zone}" + (" [DRY RUN]" if dry_run else "") + " ===")

    if not COOKIES_PATH.exists():
        log("⚠️  Pas de cookies Eventbrite — risque de blocage. Créer profiles/eventbrite/eb_cookies.json")
    ...
    with sync_playwright() as p:
        ...
        for page_num in range(1, args.pages + 1):
            # Eventbrite pagination via ?page=N
            url = ZONES[args.zone] + (f"?page={page_num}" if page_num > 1 else "")
            log(f"\nPage {page_num} : {url}")
            try:
                page.goto(url, timeout=30000)
                try:
                    page.wait_for_selector('[data-testid="event-card"], .eds-event-card', timeout=12000)
                except:
                    pass
                sleep_random(*DELAY_PAGES)

                # Vérifier si blocage (Cloudflare, captcha)
                if "just a moment" in page.title().lower() or "captcha" in page.content().lower():
                    log("  ⚠️  Blocage détecté — attente manuelle requise")
                    input("  Résoudre le captcha dans le navigateur, puis Entrée...")
        ...
        for i, event_url in enumerate(all_event_urls, 1):
            slug = event_url.rstrip('/').split('/')[-1][:60]
            ...
            try:
                ...
                ok = send_html(page.content(), f"eventbrite_{slug}.html",
