...
def run_zone(zone:str, dry_run: bool = False):
    cfg = get_zone_config(zone)
    zone = cfg["zone"]
    load_zone_env(cfg)

    print(f"\n🤖 Agent CM izilife — zone: {zone} — {datetime.now().strftime('%d/%m/%Y %H:%M')}" + (" [DRY RUN]" if dry_run else ""))
    ...
    for i, row in enumerate(to_process, 1):
        ...
        if dry_run:
            print("    [DRY RUN] aucune génération LLM/image, aucune écriture Excel")
            continue
...
def main():
    ...
    elif args.zone:       run_zone(args.zone, dry_run=args.dry_run)
    else:
        print("Usage:")
        print("  python cm_izilife.py --zone lille")
